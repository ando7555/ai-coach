...
Every request to my PitchMind app travels through 11 layers before it becomes a JSON response. Here's exactly what happens — from raw TCP to GraphQL result. 🔍
...
**HTTP/1.1 (1997)** — text-based protocol, one request per connection (unless keep-alive), the foundation everything still builds on. PitchMind runs on HTTP/1.1 in development.
...
If PitchMind used REST, we'd need separate endpoints for every operation:
...
PitchMind is a textbook GraphQL fit: graph database + deeply nested relationships + AI-generated content that varies per request.

[INSERT DIAGRAM 2: "REST vs GraphQL" — diagrams-post-5.html]

---

## 3. The Spring MVC Request Lifecycle — Traced Through PitchMind
...
PitchMind uses **schema-first** design: you write the schema in `.graphqls` files, and Spring maps it to Java methods. The alternative is **code-first**, where you annotate Java classes and the schema is generated.
...
PitchMind uses `extend type Query` and `extend type Mutation` to add operations without modifying the original block:
...
[INSERT DIAGRAM 6: "PitchMind API Map" — diagrams-post-5.html]
...
CSRF attacks exploit automatic credential attachment — browsers automatically send cookies with every request to a domain. PitchMind uses JWT in the Authorization header, which the browser never sends automatically. No automatic credentials = no CSRF risk. Enable CSRF if you switch to cookie-based session authentication.
...
**5. Why does PitchMind use `permitAll()` on `/graphql` instead of requiring authentication at the URL level?**
